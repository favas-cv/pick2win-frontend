          ...
          <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows="3"
                className="w-full bg-slate-900/60 border border-slate-800 rounded-xl py-2.5 px-4 text-xs text-white placeholder-slate-600 focus:border-sports-green focus:outline-none transition resize-none"
              />
            </div>
      ...
      {tournaments.length > 0 ? (
        <div className="space-y-3">
          {tournaments.map((t) => (
            <div
              key={t.id}
              className="glass-card border-slate-800 rounded-2xl p-5 hover:border-slate-700 flex flex-col sm:flex-row justify-between sm:items-center gap-4 transition duration-200"
            >
              {/* Left: icon + info */}
              <div className="flex items-center gap-4">
                <div className="w-11 h-11 bg-slate-900 border border-slate-850 rounded-xl flex items-center justify-center shrink-0">
                  <Trophy className="w-5 h-5 text-sports-green" />
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-sm font-extrabold text-white leading-snug">{t.name}</h3>
