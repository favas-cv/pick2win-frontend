...
export const LoadingSkeleton = ({ type = 'card', count = 1 }) => {
  const renderSkeleton = (key) => {
    switch (type) {
      case 'stat':
        return (
          <div key={key} className="bg-slate-50 border border-slate-200 rounded-2xl p-6 animate-pulse">
            <div className="w-1/3 h-3 bg-slate-100 rounded mb-4"></div>
            <div className="w-2/3 h-8 bg-slate-100 rounded"></div>
          </div>
        );
      case 'table':
        return (
          <div key={key} className="space-y-3 animate-pulse">
            <div className="h-10 bg-slate-50 rounded-xl border border-slate-200"></div>
      ...
      default:
        return (
          <div key={key} className="glass-card border-slate-200 rounded-2xl p-5 animate-pulse space-y-4">
